...
class OAHashTable
{
  ...
  private: // Some suggestions (You don't have to use any of this.)
    ...
    void GrowTable() throw(OAHashTableException);
    ...
    void RemoveHelp(OAHTSlot &to_remove) throw(OAHashTableException);
    int IndexHelp(const unsigned &hash, const unsigned &stride, const unsigned& index) const;

    unsigned BetaHash(const char* key) const;
    unsigned AlphaHash(const char* key) const;
